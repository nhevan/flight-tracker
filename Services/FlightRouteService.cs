    ...
    private async Task<FlightRoute?> FetchAndCacheAsync(string callsign, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var route = new FlightRoute(
                ...
                OriginCity:      NullIfEmpty(detail.Origin?.Municipality),
                OriginCountry:   NullIfEmpty(detail.Origin?.CountryName),
                OriginLat:       oLat,
                OriginLon:       oLon,
                ...
                DestCity:        NullIfEmpty(detail.Destination?.Municipality),
                DestCountry:     NullIfEmpty(detail.Destination?.CountryName),
                DestLat:         dLat,
                DestLon:         dLon,
