...
public sealed class MapboxSnapshotService : IMapSnapshotService
{
        ...
        IReadOnlyList<(double Lat, double Lon)>? recordedDots = null)
    {
        ...
        try
        {
            ...
            string url = $"https://api.mapbox.com/styles/v1/{style}/static" +
                         ...
                         $"{zoom},{_settings.BearingOverride ?? 350}" +
        ...
        IReadOnlyList<(double Lat, double Lon)>? recordedDots = null)
    {
        ...
        if (headingDegrees is null)
        {
            ...
            {
                var dotFeatures = BuildDotFeatures(recordedDots, maxDots: 3);
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Build GeoJSON FeatureCollection with one point marker per dot (downsampled to ≤ 15)
            var dotFeatures = BuildDotFeatures(dots, maxDots: 15);
    ...
    /// <summary>
    /// Builds a JsonArray of GeoJSON Feature objects, each a Simplestyle point marker
    ...
    private static JsonArray BuildDotFeatures(
        IReadOnlyList<(double Lat, double Lon)> dots, int maxDots)
    {
        var features = new JsonArray();
